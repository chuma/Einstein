...
//		pProject->AddChild(src);
		...
		{
			TWSNewtonScriptDocument *src = new TWSNewtonScriptDocument(this);
			//src->SetName("ROMVersion.txt");
			src->AssociateFile("/Users/matt/Desktop/Newton/NewtonDev/ROMVersion.txt");
			src->GetText();
		...
		pProject->AddChild(products);
		{
			...
			pkg->SetName("ROMVersion.pkg");
			pkg->AssociateFile("/Users/matt/Desktop/Newton/NewtonDev/ROMVersion.pkg");
...
void TWorkshop::CompileAndRun()
{
	...
	TWSProject *project = GetProject();
	if (!project) return;
	if (project->Compile()==0 && project->Upload()==0)
		project->Run();
